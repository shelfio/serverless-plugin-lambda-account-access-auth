        ...
        return {
          naming: {
            getLambdaLogicalId(functionName) {
              return `${functionName
                .charAt(0)
                .toUpperCase()}${functionName.slice(1)}LambdaFunction`;
            },
          },
        };
      },
...
describe("serverless-plugin-lambda-account-access", function () {
  ...
  describe("#beforeDeploy", function () {
    it("should not add resources when no functions defined", function () {
      ...
      expect(instance).to.have.nested.property("serverless.service.resources")
        .that.is.undefined;
    });
    ...
    it("should throw when function references access group that does not exist", function () {
      ...
      expect(() => instance.beforeDeploy()).to.throw(
        'Function "function1" references an access group "api" that does not exist'
      );
    });
    ...
    it("should log warning when group is not used", function () {
      ...
      sinon.assert.calledWithExactly(
        stub,
        '[serverless-plugin-lambda-account-access]: WARNING: Group "api2" is not used'
      );
    });
    ...
    describe("role", function () {
      it("should throw when role names are not unique", function () {
        ...
        expect(() => instance.beforeDeploy()).to.throw(
          "Roles must have unique names [foo]"
        );
      });
                  ...
                  Statement: [
                    {
                      ...
                      Action: "sts:AssumeRole",
                      Principal: {
                        AWS: [{ "Fn::ImportValue": "output-role-arn" }],
                      },
                    },
                  ],
                },
      ...
      it("should support service: true in policy", function () {
        const instance = createTestInstance({
          provider: {
            access: {
              groups: {
      ...
      it("should support sourceArns in policy", function () {});
    });
  });
});
